...
const renderMockCards = (navigation) => {
  return (
    mockCard.map((card, i) => {
     return(
       <CardAbout acion={() =>  navigation.navigate(card.subject) } key={i} link={card.subject } imageurl={card.imageurl} header={mockCard[i].header} secondHeader={mockCard[i].secondHeader} text={mockCard[i].text}  />
  )}))
}
